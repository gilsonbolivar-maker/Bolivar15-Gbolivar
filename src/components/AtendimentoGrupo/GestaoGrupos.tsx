      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs">
        <div>
          <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
            <Users className="w-5 h-5 text-lilas-600" />
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 px-3.5 py-2.5 bg-lilas-600 hover:bg-lilas-500 text-white text-xs font-semibold rounded-xl shadow transition-colors"
          >
      ...
      <div className="flex border-b border-slate-200 bg-white px-4 rounded-xl">
        <button
          ...
          className={`py-3 px-4 text-xs font-bold border-b-2 transition-colors ${
            subTab === "grupos"
              ? "border-lilas-600 text-lilas-700"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {grupos.map((grp) => {
            ...
            return (
              <div
                key={grp.id}
                className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs space-y-4 hover:border-lilas-300 transition-all flex flex-col justify-between"
              >
                <div className="space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    <span className="text-xs font-bold px-2.5 py-0.5 rounded-full bg-lilas-50 text-lilas-700 border border-lilas-200/60">
                      {grp.categoria}
                    </span>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                      <div
                        className="bg-lilas-600 h-full transition-all"
                ...
                <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-lilas-50 hover:bg-lilas-100 text-lilas-700 text-xs font-semibold rounded-lg transition-colors"
                  >
                  ...
                  <button
                    onClick={onOpenNovaSessao}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-lilas-600 hover:bg-lilas-500 text-white text-xs font-semibold rounded-lg shadow transition-colors"
                  >
              ...
              return (
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-slate-100">
                    <div>
                      <span className="text-xs font-bold text-lilas-700 uppercase tracking-wider">
                        {ses.grupoNome}
                      </span>
                  ...
                  <div className="p-3 bg-lilas-50/40 rounded-xl border border-lilas-100 text-xs">
                    <span className="font-bold text-lilas-950 block mb-1">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-xl border border-slate-200 max-w-md w-full space-y-4">
            ...
            <form onSubmit={handleInscreverSubmit} className="space-y-4 text-xs">
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  className="px-4 py-1.5 bg-lilas-600 hover:bg-lilas-500 text-white font-semibold rounded-lg shadow"
                >
                  Confirmar Inscrição
                </button>
              </div>
