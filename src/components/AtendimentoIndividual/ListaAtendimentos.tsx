      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs">
        <div>
          <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
            <UserCheck className="w-5 h-5 text-lilas-600" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="sm:col-span-2 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-lilas-500 outline-none"
          />
        </div>
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-slate-100">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-2.5 py-0.5 bg-lilas-50 text-lilas-800 border border-lilas-200/80 text-xs font-semibold rounded-md">
                      {a.tipoAtendimento}
                    </span>
                ...
                <div className="space-y-2">
                  ...
                  {(a.objetivos ||
                    ...
                    a.proximosPassos) && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs bg-lilas-50/40 border border-lilas-100 rounded-xl p-3">
                      {a.objetivos && (
                        <p>
                          <strong className="text-lilas-800">Objetivos:</strong>{" "}
                          <span className="text-slate-600">{a.objetivos}</span>
                        </p>
                      )}
                      {a.planoIntervencao && (
                        <p>
                          <strong className="text-lilas-800">Plano de Intervenção:</strong>{" "}
                          <span className="text-slate-600">{a.planoIntervencao}</span>
                        </p>
                      )}
                      {a.tecnicasUtilizadas && a.tecnicasUtilizadas.length > 0 && (
                        <p className="sm:col-span-2">
                          <strong className="text-lilas-800">Técnicas:</strong>{" "}
                          <span className="text-slate-600">{a.tecnicasUtilizadas.join(", ")}</span>
                        </p>
                      )}
                      {a.respostaCrianca && (
                        <p>
                          <strong className="text-lilas-800">Resposta do Aluno:</strong>{" "}
                          <span className="text-slate-600">{a.respostaCrianca}</span>
                        </p>
                      )}
                      {a.orientacoesFornecidas && (
                        <p>
                          <strong className="text-lilas-800">Orientações:</strong>{" "}
                          <span className="text-slate-600">{a.orientacoesFornecidas}</span>
                        </p>
                      )}
                      {a.proximosPassos && (
                        <p className="sm:col-span-2">
                          <strong className="text-lilas-800">Próximos Passos:</strong>{" "}
                ...
                <div className="pt-2 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs text-slate-500">
                  ...
                  {pac && (
                    <button
                      ...
                      className="inline-flex items-center gap-1 text-lilas-700 hover:text-lilas-900 font-semibold hover:underline"
                    >
